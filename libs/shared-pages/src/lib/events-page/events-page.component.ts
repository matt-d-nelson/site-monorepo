...
export class EventsPageComponent implements OnInit {
  ...
  createEvent() {
    ...
    this.eventsService
      .createEvent(this.orgId(), eventForm.value)
  ...
  updateEvent() {
    ...
    const eventDif = GetObjectDifference(
      this.previousEventValue(),
      eventUpdateForm.value
    )
    if (isEmpty(eventDif)) {
    ...
    this.dialogLoading.set(true)
    this.eventsService
      .updateEvent(this.orgId(), eventUpdateForm.get('id').value, eventDif)
      .pipe(finalize(() => this.dialogLoading.set(false)))
      .subscribe({
        next: () => {
          this.toastService.showToast(successMsg)
